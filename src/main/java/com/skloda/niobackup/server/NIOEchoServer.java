package com.skloda.niobackup.server;
...
class SocketClientChannelThread implements Runnable {
    ...
    SocketClientChannelThread(SocketChannel clientChannel) {
...
class EchoServerHandle implements AutoCloseable {    // 定义服务器端的服务处理类
    ...
    public EchoServerHandle() throws Exception {
        ...
        this.serverSocketChannel.bind(new InetSocketAddress(ServerInfo.SERVER_PORT));
        ...
        System.out.println("服务器端程序启动，该程序在" + ServerInfo.SERVER_PORT + "端口上进行监听...");
        this.clientHandle();
    }

    private void clientHandle() throws Exception {
        while (this.selector.select() > 0) {  // 需要进行连接等待
